...
#[wasm_bindgen]
pub fn get_coordinate(width: u32,index: usize) -> Position{
	...
	fn make_movement(&mut self){
		...
		for cell in self.cells.iter(){
			if cell.status == Life::Alive && cell.stamina >= 1{
				...
				match cell_index {
					Some(old_index) => {
						let chosen_direction:Direction = Direction::North;
						...
						let minus_stamina: u8 = if chosen_direction == Direction::Stand {0} else {1};
						let creature_in_new_place: Cell = self.create_cell_for_new_position(new_index,old_index, inside_in_new_place,minus_stamina);
...
impl Universe {
	...
	fn set(&mut self,new_creature: Organizm){
		let mut next = self.cells.clone();
	...
	pub fn create_test_sheep(&mut self){
		let ship_1:Organizm = Organizm{
